...
from strategies import (
    Context, BuyHoldBTC, EqualWeight, DCA,
...
def strategy_factories(capital: float) -> dict:
    ...
    return {
        ...
        # --- いま実運用に載せているもの ---
        "レジーム切替(実運用)": lambda: RegimeSwitching(),
    }
...
def save_trades(name: str, broker: Broker, tag: str) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8-sig") as f:
        ...
        for t in broker.trades:
            w.writerow([t.date.date(), t.symbol, t.side, f"{t.qty:.8f}",
                        f"{t.price:,.0f}", f"{t.amount_jpy:,.0f}",
...
def run_period(label: str, start: str | None, end: str | None,
    ...
    for name, factory in strategy_factories(CAPITAL).items():
        ...
        kd = None if name in ("BTC買い持ち", "均等分散(月次)", "分割投入(12ヶ月)") else kill_dd
    ...
    if not quiet:
        print(metrics_mod.format_table(results, benchmark="BTC買い持ち"))
        os.makedirs(RESULTS_DIR, exist_ok=True)
        curve_path = os.path.join(RESULTS_DIR, f"equity_{label}.csv")
        pd.DataFrame(curves).round(0).to_csv(curve_path, encoding="utf-8-sig")
        print(f"\n資産曲線: {curve_path}")
...
def main() -> None:
    ...
    print(f"手数料 {0.15}% / スリッページ {0.10}% / 現物のみ（ショート・レバレッジなし）")
