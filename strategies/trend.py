...
トレンドフォロー系
------------------------------------------------------------
暗号資産は「大きなトレンドを取り、崩れたら降りる」性質が強いので本命はこちら。
"""
...
class FilteredEqualWeight(Strategy):
    """
    自分の移動平均を上回っている銘柄だけを均等保有する。
    強気相場で「持っているだけ」に近い挙動をしつつ、崩れた銘柄は外す。
    """
...
class SMACross(Strategy):
    """
    移動平均クロスの順張り。短期>長期の銘柄を均等保有。
    既存 paper_trade.py と同じ発想。レンジ相場でのダマシがどれだけ効くかを見る比較用。
    """
