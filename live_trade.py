...
実行:
  ...
  python live_trade.py --once          # 1回だけ実行して終了
  python live_trade.py --reset         # 元手100万円から仕切り直す
  python live_trade.py --status        # 現状だけ表示して終了
...
停止条件:
  ...
  ※ 現物のみ・ショートなしなので、保有4銘柄すべてが無価値にならない限り0円には到達しない。
     実質的な撤退ラインを設けたい場合は --stop-below を明示的に指定すること。
...
class LiveTrader:
    ...
    def _guard(self, panel: dict, prices: dict) -> dict:
        ...
        「建玉後の最高値からATR×3」下げた銘柄は目標ウェイトを0にする。
        """
        ...
        with open(TRADES_LOG, "a", newline="", encoding="utf-8-sig") as f:
            ...
            if is_new_file:
                w.writerow(["日時", "銘柄", "売買", "数量", "約定単価",
                            "金額(円)", "実現損益(円)", "理由"])
            for t in new:
                w.writerow([t.date, t.symbol, t.side, f"{t.qty:.8f}",
    ...
    def step(self, verbose: bool = True) -> bool:
        ...
        if not prices:
            print(f"[{ts}] 現在値を取得できませんでした。次のループで再試行します。")
            return True
